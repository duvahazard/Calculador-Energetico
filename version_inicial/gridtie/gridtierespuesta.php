 ...
 */
function crear_tabla_gtrespuesta($con, $idterreno, $idcaso) {

	// Obtener con un Query sobre la tabla de casos solo los que sean de tipo 4 (gridTie)
	$gridties    = getGridTies( $con, $idterreno, $idcaso );
	$efectividad = Array();

	$total_gridties = count ($gridties);
	$total_fvs      = 0;
	$idfv           = "";
	$anyo_fvr       = getAnyoCamino_solar( $con, $idterreno );
	$minMaxAnyo     = getMinMaxAnyoHorasDelMes($con);
	$deltaT         = getDeltaT( $con, $idterreno );
	$cantidad_fvs   = 0;
	...
	for( $i = 0; $i < $total_gridties; $i ++ ) {
			...
			crearTabla( $con, $tabla_salida );

			// leer el factor de efectividad de tabla ce_dispositivo, el 2do factor que es porcentaje y guardarlo en una variable
			$efectividad  = getEfectividad( $con, $efectividad, $gridties[$i]["marca"] );// leer el factor de efectividad
			...
			for( $j = 0; $j < $total_fvs; $j ++ ) {
				...
				$cantidad_fvs = getCantidadFVs( $con, $idterreno, $idfv );

				if(!respuesta_fv( $con, $idterreno, $idfv ) ) {
				...
				for( $mes = 1; $mes <=13; $mes ++){ // Ciclar por cada mes, empezando desde enero
					...
					if($mes==13){ // caso cuando sea leap year, sumar febrero pero con el 28 contado 2 veces
						$respuesta_sumas = getSumaValoresFVR_LY( $con, $idterreno, $idcaso, $idfv, $anyo_fvr, $efectividad[$gridties[$i]["marca"] ], $deltaT, $cantidad_fvs );
					} else {
						$respuesta_sumas = getSumaValoresFVR( $con, $idterreno, $idcaso, $idfv, $mes, $anyo_fvr, $efectividad[$gridties[$i]["marca"] ], $deltaT, $cantidad_fvs );
					}
		    	...
		    	for( $gridMes = 1; $gridMes <= 12; $gridMes ++ ) { // cycle through all months and set the same production values for each.
					...
					if($gridMes==2){//leave in only leap years (2012,2016,2020,2024,2028,2032,2036,2040,2044,2048,2052,2056,2060,2064,2068,2072,2076,2080,2084,2088,2092,2096)
						...
						if( esBisiesto( $gridAnyo ) ) {
							insertaRegistroGT( $con, $tabla_salida, $gridAnyo, $gridMes, $mesTotalCS[13], $mesTotalCL[13] );
						} else {
							insertaRegistroGT( $con, $tabla_salida, $gridAnyo, $gridMes, $mesTotalCS[$gridMes], $mesTotalCL[$gridMes] );
...
function insertaRegistroGT( $con, $nombre_tabla, $anyo, $mes, $cs, $cl ) {
...
function getCantidadFVs( $con, $idterreno, $idfv ) {
...
function getMinMaxAnyoHorasDelMes($con) {
...
function getSumaValoresFVR_LY( $con, $idterreno, $idcaso, $idfv, $anyo_fvr, $eff, $deltaT, $cantidad_fvs ) {
...
function getDeltaT( $con, $idterreno ) {
...
function getSumaValoresFVR( $con, $idterreno, $idcaso, $idfv, $mes, $anyo_fvr, $eff, $deltaT, $cantidad_fvs ) {
...
function respuesta_fv( $con, $idterreno, $idfv ) {
...
function getGridTies( $con, $idterreno, $idcaso ) {
...
function getAnyoCamino_solar( $con, $idterreno ) {
	$anyo = "0";
...
function crearTabla( $con, $nombre_tabla ) {
...
// leer el factor de efectividad de tabla ce_dispositivo, el 2do factor que es porcentaje y guardarlo en una variable
function getEfectividad( $con, $efectividad, $iddispositivo ) {
